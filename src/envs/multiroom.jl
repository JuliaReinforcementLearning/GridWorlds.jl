...
function placeRooms!( n, numLeft, roomList, minSize, maxSize, entryDoorWall, entryDoorPos, rng=Random.GLOBAL_RNG)
    ...
    elseif entryDoorWall == 0 #RIGHT
        top = CartesianIndex(entryDoorPos[1]-sizeX+1, rand(rng, entryDoorPos[2]-sizeY+2:entryDoorPos[2]-1))
    elseif entryDoorWall == 1 #BOTTOM
        top = CartesianIndex(rand(rng, entryDoorPos[1]-sizeX+2:entryDoorPos[1]-1), entryDoorPos[2]-sizeY+1)
    elseif entryDoorWall == 2 #LEFT
        top = CartesianIndex(entryDoorPos[1], rand(rng, entryDoorPos[2]-sizeY+2:entryDoorPos[2]-1))
    elseif entryDoorWall == 3 #TOP
        top = CartesianIndex(rand(rng, entryDoorPos[1]-sizeX+2:entryDoorPos[1]-1), entryDoorPos[2])
    else
        return false
    end
    ...
    for i=1:8
        ...
        if exitDoorWall == 0
            exitDoorPos = CartesianIndex(top[1]+sizeX-1,top[2]+rand(rng, 1:sizeY-2))
        elseif exitDoorWall == 1
            exitDoorPos = CartesianIndex(top[1]+rand(rng, 1:sizeX-2),top[2]+sizeY-1)
        elseif exitDoorWall == 2
            exitDoorPos = CartesianIndex(top[1],top[2]+rand(rng, 1:sizeY-2))
        elseif exitDoorWall == 3
            exitDoorPos = CartesianIndex(top[1]+rand(rng, 1:sizeX-2),top[2])
        else
            return false
        end
...
function MultiRoom(;n=25, minRooms=2, maxRooms=4, maxSize=10, rng=Random.GLOBAL_RNG)
    ...
    while length(roomList) < numRooms
        curRoomList = []
        entryDoorPos = CartesianIndex(rand(rng, 1:n-2),rand(rng, 1:n-2))
        placeRooms!(n, numRooms, curRoomList, 4, maxSize, 2, entryDoorPos)
    ...
    for (i, room) in enumerate(roomList)
        ...
        if i>1
            ...
            world[EMPTY, room.entryDoor] = false
